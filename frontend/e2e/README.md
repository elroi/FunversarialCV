...
- **Engine section:** The armed-CV line and **Harden** live inside **Engine Configuration** / **How it runs**, which starts **collapsed**. Call `expandEngineConfigurationSection` **before** `expect(...armedCvLabel).toBeVisible()`. The helper is idempotent (safe to call again if the section is already open).
